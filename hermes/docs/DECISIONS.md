...
### 2026-07-04 — Whole-codebase ultra review (post-M6)

Eight-lens review of the full codebase (spec conformance, paper fidelity, cross-module contracts, concurrency/failure paths, eval integrity, security/robustness, test quality, docs drift); every finding adversarially verified before fixing. Fixes and rulings:

- **Budget honesty (fixed):** a billed refusal (`parsed_output=None`) now attaches its real UsageRecord to `DetectorFailure` (previously that spend was invisible to the meter and run.json); failed consolidations now `meter.add` their estimated spend like failed detections.
- **Consolidation cache key (fixed):** `CONSOLIDATOR_PROMPT_VERSION` now lives beside `CONSOLIDATOR_SYSTEM` in llm.py (bump on edit, like PROMPT_VERSIONS), and the key hashes BOTH message parts (endpoint summary + findings JSON). Pre-review consolidation cache entries are orphaned — harmless, local cache.
- **Cache degrades, never crashes (fixed):** a cached blob that fails current-schema validation, and sqlite `OperationalError` on read/write (shared cache.db, second process), are treated as misses / dropped writes instead of aborting the scan with a raw traceback.
- **Reduction failures are per-operation (fixed):** `node_reduce` records a `_reducer` error and skips the op (no ERD → no fan-out → no spurious clean verdict; `node_collect` keys off built ERDs). Hostile top-level values (`securityDefinitions`/`securitySchemes`/`tags` of the wrong type), unbounded inline schema nesting (structural depth cap 60 → `$truncated`), and parser `RecursionError` (now a `SpecError`, exit 2) no longer kill scans.
- **Dashboard embedding hardened (fixed):** `_embed` now escapes every `<` as `\u003c` (the old `</`-only rule blocked `</script>` breakout but a crafted `info.title` containing `<!--<script ` could still kill the dashboard JS via the HTML script-data-double-escaped state — reproduced in-browser during verification). Severity is also `esc()`d in its class attribute (defense-in-depth; the enum already prevents injection).
- **Flake-shield consistency (fixed):** retry keys whose call FAILED keep the base run's prediction (absence of a failed key is missing data, not a negative). Previously an API flake during the retry could flip gates to PASS while the committed baseline still contained the base run's contradicting `smell_detected=true` records — the reported metrics were irreproducible from the recordings (verified by reproduction).
- **Offline-eval staleness (fixed):** unknown `smell_id` in recordings and a recorded-vs-configured detect-model mismatch now raise `StaleRecordingsError` (the model check was promised in the module docstring but unimplemented).
- **Resume scope + stale artifacts (fixed):** run.json now echoes the endpoint filters (`tags/paths/sample/max_endpoints`); `hermes report` warns on `status != completed` because an interrupted re-run rewrites only run.json and sibling artifacts may belong to an earlier completed run under the same run-id.
- **Verdict trailer is a deliberate extension:** the Appendix-C markdown (and its JS mirror) ends with `Verdict: <verdict> (<note>)` — not part of the paper's report shape; kept for triage value, now logged. 00-SPEC §7.1's format block remains the paper shape; the trailer is additive.
- **ERD document-level context is a logged deviation:** `api_title`, capped `info.description`, tag descriptions, and operation metadata ride along in the ERD context (00-SPEC §5 amended) — SECURITY/RESPONSE need document conventions to avoid false positives.
- **Accepted residual risks:** YAML anchor/alias amplification (billion-laughs) in `yaml.safe_load` is unmitigated for `.yaml` specs — pyyaml has no alias limit; acceptable for a local CLI whose user chooses the input file. The throttle re-check inside the semaphore fixes a latent (currently unreachable) 429-bypass. Subprocess CLI tests now strip `ANTHROPIC_*`/`HERMES_*` env and plant a canary key.
- **Refuted by adversarial verification (no change):** Ctrl-C cannot orphan in-flight cache writes (langgraph's executor drains workers before unwinding); `drop_duplicate` deleting a cross-smell record is within §8's merge license and the consolidator prompt constrains it to same-defect redundancy; exception reprs in run.json don't leak proxy credentials (httpx reprs carry only OS-level messages); the exact-literal "Get data" prompt/fixture overlap is paper-sourced and explicitly permitted by the M3 eval-integrity rule.
- Doc drift corrected across 00/01/02/03/04 (graph node set, ScanState shape, wholesale-write persistence, clean-resend retry, no-cache-savings cost model, `render_run(store)` signature, recordings layout, `HERMES_OUT`, M4 acceptance command, finding-id recipe, `--resume --run-id` contract, best-effort token cap).
